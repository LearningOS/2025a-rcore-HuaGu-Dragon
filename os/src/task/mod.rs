...
use crate::task::task::SystemCallCount;
use lazy_static::*;
use switch::__switch;
...
pub struct TaskManagerInner {
    ...
    current_task: usize,
    tasks_called: [SystemCallCount; MAX_APP_NUM],
}
        ...
        TaskManager {
            ...
            inner: unsafe {
                UPSafeCell::new(TaskManagerInner {
                    tasks,
                    current_task: 0,
                    tasks_called: [SystemCallCount::new(); MAX_APP_NUM],
                })
            },
        }
    };
...
impl TaskManager {
    ...
    fn task_syscall(&self, id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks_called[current].syscall_counts[id] += 1;
    }

    fn get_task_syscall(&self, id: usize) -> usize {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks_called[current].syscall_counts[id]
    }
}
